<x-app-layout>
    <div x-data="{
        editOpen: false,
        editTask: { id: null, title: '', description: '', priority: 'medium', importance: 'none', status: 'todo', due_date: '', is_recurring: false, tag_ids: [], project_id: '', subtasks: [], comments: [] },
        tasks: {{ Js::from(collect($quadrants)->flatten()->map(fn($t) => [
            'id' => $t->id,
            'title' => $t->title,
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-white flex-shrink-0">
            <div>
                <h1 class="text-lg font-semibold text-gray-900">Eisenhower Matrix</h1>
                <p class="text-xs text-gray-400 mt-0.5">Drag tasks between quadrants to reprioritize</p>
            </div>
        </div>
            ...
            <div class="h-full flex flex-col">
                ...
                <div class="grid grid-cols-2 gap-3 mb-1">
                    <div class="text-center">
                        <span class="text-[10px] font-bold uppercase tracking-widest text-red-500">Urgent</span>
                    </div>
                    <div class="text-center">
                        <span class="text-[10px] font-bold uppercase tracking-widest text-blue-500">Not Urgent</span>
                    </div>
                </div>

                <div class="flex-1 flex gap-3 min-h-0">
                    {{-- Left axis: Importance --}}
                    <div class="flex flex-col justify-center w-6 flex-shrink-0">
                        <span class="text-[10px] font-bold uppercase tracking-widest text-emerald-600 -rotate-90 whitespace-nowrap origin-center">Important</span>
                        <div class="flex-1"></div>
                        <span class="text-[10px] font-bold uppercase tracking-widest text-gray-400 -rotate-90 whitespace-nowrap origin-center">Not Important</span>
                    </div>
                    ...
                    <div class="flex-1 grid grid-cols-2 grid-rows-2 gap-3 min-h-0">
                        ...
                        <div class="flex flex-col rounded-xl border-2 border-red-200 bg-red-50/50 min-h-0">
                            <div class="flex items-center justify-between px-3 py-2 border-b border-red-200/60 flex-shrink-0">
                                <div class="flex items-center gap-2">
                                    <div class="w-2 h-2 rounded-full bg-red-500"></div>
                                    <h3 class="text-xs font-bold text-red-700 uppercase tracking-wide">Do</h3>
                                </div>
                                <span class="text-[10px] font-semibold text-red-500 bg-red-100 rounded-full px-1.5 py-0.5"
                                    x-text="getQuadrantTasks('do').length"></span>
                            </div>
                            <div class="flex-1 overflow-y-auto p-2 space-y-1.5" data-quadrant="do">
                                <template x-for="task in getQuadrantTasks('do')" :key="task.id">
                                    <div class="bg-white rounded-lg border border-red-200/60 p-2.5 shadow-sm hover:shadow-md transition cursor-grab"
                                         :data-id="task.id" @click="openEdit(task)">
                                        <p class="text-sm font-medium text-gray-900 leading-snug" x-text="task.title"></p>
                                ...
                                <div x-show="getQuadrantTasks('do').length === 0" class="text-center text-xs text-red-300 py-4">No tasks</div>
                            </div>
                        </div>

                        {{-- Q2: Schedule (Not Urgent + Important) --}}
                        <div class="flex flex-col rounded-xl border-2 border-blue-200 bg-blue-50/50 min-h-0">
                            <div class="flex items-center justify-between px-3 py-2 border-b border-blue-200/60 flex-shrink-0">
                                <div class="flex items-center gap-2">
                                    <div class="w-2 h-2 rounded-full bg-blue-500"></div>
                                    <h3 class="text-xs font-bold text-blue-700 uppercase tracking-wide">Schedule</h3>
                                </div>
                                <span class="text-[10px] font-semibold text-blue-500 bg-blue-100 rounded-full px-1.5 py-0.5"
                                    x-text="getQuadrantTasks('schedule').length"></span>
                            </div>
                            <div class="flex-1 overflow-y-auto p-2 space-y-1.5" data-quadrant="schedule">
                                <template x-for="task in getQuadrantTasks('schedule')" :key="task.id">
                                    <div class="bg-white rounded-lg border border-blue-200/60 p-2.5 shadow-sm hover:shadow-md transition cursor-grab"
                                         :data-id="task.id" @click="openEdit(task)">
                                        <p class="text-sm font-medium text-gray-900 leading-snug" x-text="task.title"></p>
                                        <div class="flex items-center gap-2 mt-1.5">
                                            <span class="text-[10px] px-1.5 py-0.5 rounded font-semibold uppercase tracking-wide"
                                ...
                                <div x-show="getQuadrantTasks('schedule').length === 0" class="text-center text-xs text-blue-300 py-4">No tasks</div>
                            </div>
                        </div>

                        {{-- Q3: Delegate (Urgent + Not Important) --}}
                        <div class="flex flex-col rounded-xl border-2 border-orange-200 bg-orange-50/50 min-h-0">
                            <div class="flex items-center justify-between px-3 py-2 border-b border-orange-200/60 flex-shrink-0">
                                <div class="flex items-center gap-2">
                                    <div class="w-2 h-2 rounded-full bg-orange-500"></div>
                                    <h3 class="text-xs font-bold text-orange-700 uppercase tracking-wide">Delegate</h3>
                                </div>
                                <span class="text-[10px] font-semibold text-orange-500 bg-orange-100 rounded-full px-1.5 py-0.5"
                                    x-text="getQuadrantTasks('delegate').length"></span>
                            </div>
                            <div class="flex-1 overflow-y-auto p-2 space-y-1.5" data-quadrant="delegate">
                                <template x-for="task in getQuadrantTasks('delegate')" :key="task.id">
                                    <div class="bg-white rounded-lg border border-orange-200/60 p-2.5 shadow-sm hover:shadow-md transition cursor-grab"
                                         :data-id="task.id" @click="openEdit(task)">
                                        <p class="text-sm font-medium text-gray-900 leading-snug" x-text="task.title"></p>
                                        <div class="flex items-center gap-2 mt-1.5">
                                            <span class="text-[10px] px-1.5 py-0.5 rounded font-semibold uppercase tracking-wide"
                                ...
                                <div x-show="getQuadrantTasks('delegate').length === 0" class="text-center text-xs text-orange-300 py-4">No tasks</div>
                            </div>
                        </div>

                        {{-- Q4: Delete (Not Urgent + Not Important) --}}
                        <div class="flex flex-col rounded-xl border-2 border-gray-200 bg-gray-50/50 min-h-0">
                            <div class="flex items-center justify-between px-3 py-2 border-b border-gray-200/60 flex-shrink-0">
                                <div class="flex items-center gap-2">
                                    <div class="w-2 h-2 rounded-full bg-gray-400"></div>
                                    <h3 class="text-xs font-bold text-gray-500 uppercase tracking-wide">Delete</h3>
                                </div>
                                <span class="text-[10px] font-semibold text-gray-500 bg-gray-200 rounded-full px-1.5 py-0.5"
                                    x-text="getQuadrantTasks('delete').length"></span>
                            </div>
                            <div class="flex-1 overflow-y-auto p-2 space-y-1.5" data-quadrant="delete">
                                <template x-for="task in getQuadrantTasks('delete')" :key="task.id">
                                    <div class="bg-white rounded-lg border border-gray-200/60 p-2.5 shadow-sm hover:shadow-md transition cursor-grab"
                                         :data-id="task.id" @click="openEdit(task)">
                                        <p class="text-sm font-medium text-gray-900 leading-snug" x-text="task.title"></p>
                                        <div class="flex items-center gap-2 mt-1.5">
                                            <span class="text-[10px] px-1.5 py-0.5 rounded font-semibold uppercase tracking-wide"
                                ...
                                <div x-show="getQuadrantTasks('delete').length === 0" class="text-center text-xs text-gray-300 py-4">No tasks</div>
                            </div>
                        </div>
                ...
                new Sortable(col, {
                    ...
                    onEnd: function (evt) {
                        ...
                        fetch(`/tasks/${taskId}/eisenhower`, {
                            ...
                            if (Alpine) {
                                const data = Alpine.$data(evt.to.closest('[x-data]'));
                                if (data) {
                                    const task = data.tasks.find(t => t.id == taskId);
                                    if (task) {
                                        task.quadrant = newQuadrant;
                                        if (newQuadrant === 'do' || newQuadrant === 'delegate') {
                                            task.due_date = new Date().toISOString().split('T')[0];
                                        }
                                        if (newQuadrant === 'do' || newQuadrant === 'schedule') {
                                            task.importance = 'high';
                                        } else {
                                            task.importance = 'low';
                                        }
                                        if (newQuadrant === 'delete') {
                                            task.due_date = '';
                                        }
                                    }
                                }
