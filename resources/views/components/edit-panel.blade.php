...
<div x-show="editOpen" x-cloak
    ...
    <div class="flex items-center gap-3 px-6 py-3 border-b border-gray-200 flex-shrink-0">
        ...
        <div class="relative" x-data="{ flagOpen: false }" @click.outside="flagOpen = false">
            <button @click="flagOpen = !flagOpen" class="p-1.5 rounded-lg transition hover:bg-gray-100">
        ...
        <div x-show="commentsOpen" x-cloak x-transition class="px-6 py-4 border-t border-gray-200">
            <div class="space-y-3 mb-3">
                <template x-for="comment in editTask.comments" :key="comment.id">
                    <div class="flex gap-2">
                        <div class="w-6 h-6 rounded-full bg-brand-50 text-brand-600 flex items-center justify-center text-xs font-semibold flex-shrink-0 mt-0.5"
            ...
            <form method="POST" x-bind:action="`/tasks/${editTask.id}/comments`" class="flex gap-2">
                ...
                <textarea name="body" required maxlength="1000" placeholder="Write a comment..."
                    ...
                    rows="2"></textarea>
                <button type="submit" class="bg-brand-500 hover:bg-brand-600 text-white text-xs font-medium px-3 py-1.5 rounded-lg transition self-end">Send</button>
            </form>
        </div>
    </div>
    ...
    <div class="flex items-center justify-end gap-1 px-4 py-2.5 border-t border-gray-200 flex-shrink-0">
        <button @click="commentsOpen = !commentsOpen"
            ...
            :class="commentsOpen ? 'bg-brand-50 text-brand-600' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50'">
        ...
        <div class="relative">
            <button @click="menuOpen = !menuOpen"
                class="p-2 rounded-lg transition"
                :class="menuOpen ? 'bg-brand-50 text-brand-600' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50'">
            ...
            <div x-show="menuOpen" x-cloak @click.outside="menuOpen = false"
                ...
                <div class="p-3">
                    ...
                    <form method="POST" x-bind:action="`/tasks/${editTask.id}/subtasks`" class="flex gap-2" @click.stop>
                        ...
                        <button type="submit" class="bg-brand-500 hover:bg-brand-600 text-white text-xs font-medium px-3 py-1.5 rounded-lg transition whitespace-nowrap">Add</button>
                    </form>
                </div>

                {{-- Start Focus --}}
                <div class="p-3 border-t border-gray-100">
                    <a :href="'/pomodoro?task_id=' + editTask.id" class="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium text-brand-600 hover:bg-brand-50 transition" @click.stop>
